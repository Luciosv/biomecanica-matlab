...
% graficar_vectores_pelvis - Grafica múltiples vectores 3D desde orígenes con configuración personalizada
%
% Parámetros:
...
%   config    - Struct con campos opcionales:
...
%       .MostrarOrigen  - true/false para mostrar los puntos de origen
    ...
    if ~isfield(config, 'EtiquetaU'), config.EtiquetaU = 'u\_pelvis'; end
    if ~isfield(config, 'EtiquetaV'), config.EtiquetaV = 'v\_pelvis'; end
    if ~isfield(config, 'EtiquetaW'), config.EtiquetaW = 'w\_pelvis'; end
    if ~isfield(config, 'Titulo'), config.Titulo = 'Sistema de Coordenadas Pélvico'; end
